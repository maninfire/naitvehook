...
{
    FILE *fp;
    fp = fopen("/data/local/tmp/my_hookso.txt", "r");
    char line[256];
...
void* get_remote_addr(int target_pid, const char* module_name, void* local_addr)
{
    ...
    void* ret_addr = (void *)((ptr)local_addr + (ptr)remote_handle - (ptr)local_handle);
#if defined(__i386__)
    if (!strcmp(module_name, "/system/lib/libc.so"))
        {
            //ret_addr =(void *)((ptr)ret_addr+2);
        }
#endif
    return ret_addr;
}
...
//byte[] * newd="dssdfsfasdf";

JNICALL
Java_com_qiye_txz_naitvehook_MainActivity_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {
    std::string hello = "Hello from C++";
   // test();
    dlopen(target_soname,RTLD_NOW);
